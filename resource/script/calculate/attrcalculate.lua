...
function ExAttrCheck(role)  
	...
	local mxhp	= math.floor(HP_RAID * Con(role)*3 * Mxhp_con_rad1[job] + Mxhp_con_rad2[job] * math.pow( math.floor(Con(role)*3/20 ),  2) +Lv(role) * Mxhp_lv_rad[job] + 40)
	local mxsp	= math.floor(SP_RAID * Sta(role)*3 * Mxsp_sta_rad1[job] + Mxsp_sta_rad2[job] * math.pow( math.floor(Sta(role)*3/20), 2) +Lv(role) * Mxsp_lv_rad[job] + 5)			
	...
	local def = math.floor( DEF_RAID * Con(role)* 5 * Def_con_rad1[job]	+ Def_con_rad2[job] * math.floor( math.pow( Con(role) * 3 /20, 2) ) )											
...
function ExAttrSet(role) 
	...
	local mspd_final = Mspd_final(role) * MSPD_RAID 
