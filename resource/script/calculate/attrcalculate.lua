print( "‡ Јаг§Є  attr_calculate.lua" )
--Приросты характерстик
...
function Shengji_Shuxingchengzhang( role ) 
	...
	if lv >= LV_LIMIT then 
		...
		LG ( "Достижение КАП Уровня" , Cha_name.." "..lv.." Дата - "..year..sep..month..sep..day) 
	end 
	...
	if lv == 60 then
		...
		if cha_type == 2 then --ланс
			local attr_ap = Attr_ap( role ) 
			attr_ap = attr_ap + 1
			SetCharaAttr(attr_ap, role, ATTR_AP)  
		end
		if cha_type == 1  then -- карц
			local attr_ap = Attr_ap( role ) 
			attr_ap = attr_ap + 1
			SetCharaAttr(attr_ap, role, ATTR_AP) 
		end
	...
	if lv == 5 then
		HelpInfo(role,0,"Поздравляем! Вы достигли 5 уровня, ваш подарок во временном мешке.")
		GiveItemX( role , 0 , 1847  , 25 , 4 )
	end
	
	if lv == 10 then
		HelpInfo(role,0,"Поздравляем! Вы достигли 10 уровня, ваш подарок во временном мешке")
		
		GiveItemX( role , 0 , 1847  , 20 , 4 )
		AddMoney(role , 0, 10000)
	end
	
	if lv == 15 then
		HelpInfo(role,0,"Поздравляем! Вы достигли 15 уровня, ваш подарок во временном мешке, убивая мини-боссов вы можете повысить свою репутацию")
		GiveItemX( role , 0 , 1848  , 30 , 4 )
	end
	
	if lv == 20 then
		HelpInfo(role,0,"Поздравляем! Вы достигли 20 уровня, ваш подарок во временном мешке. Выполняйте Исторический квест и получайте очки мирных навыков")
		GiveItemX( role , 0 , 1848  , 40 , 4 )
	end
	
	if lv == 21 then
		HelpInfo(role,0,"Вы можете построить корабль, обратитесь к Корабелу в любом городе, чтобы посмотреть ассортимент кораблей. ")
	end
	
	if lv == 25 then
		HelpInfo(role,0,"Поздравляем! Вы достигли 25 уровня, ваш подарок во временном мешке, выполняйте задания у вашего наставника и получайте много опыта и денег. Так же вы можете получить Медаль отваги, обратитесь к Администратору Арены ")
		GiveItemX( role , 0 , 1849  , 40 , 4 )
		--AddMoney(role , 0, 30000)
	end
	
	if lv == 30 then
		HelpInfo(role,0,"Поздравляем! Вы достигли 30 уровня, ваш подарок во временном мешке.")
		HelpInfo(role,0,"Теперь вы можете Торговать, вы можете найти торговцев в каждом поселении.")
		GiveItemX( role , 0 , 1849  , 90 , 4 )
	end
	
	if lv == 35 then
		HelpInfo(role,0,"Поздравляем! Вы достигли 35 уровня, ваш подарок во временном мешке. Убивая существ более 30 уровня, вы можете получить свиток усиления, подробности можете узнать у НПС Чародейка в Аргенте.")
		GiveItemX( role , 0 , 1849  , 130 , 4 )
	...
	if lv == 40 then
		HelpInfo(role,0,"Поздравляем! Вы достигли 40 уровня, ваш подарок во временном мешке. Вы можете получить вторую профессию, поговорив с наставницей, в любой столице. Теперь вы посещать Темную Топь и Затерянный город, так же можете добыть фею - убейте Бехамота в шахтах.")
		HelpInfo(role,0,"")
		GiveItemX( role , 0 , 8290  , 75 , 4 )
		
		--AddMoney(role , 0, 100000)
	end
	
	if lv == 41 then
		HelpInfo(role,0,"Для того, чтобы получить навыки на вторую профессию посещайте Темную топь или Затерянный город, или находите книги в море, убивая морских существ.")
	end
	
	if lv == 45 then
		HelpInfo(role,0,"Поздравляем! Вы достигли 45 уровня, ваш подарок во временном мешке.")
		GiveItemX( role , 0 , 8290  , 90 , 4 )
		
		--AddMoney(role , 0, 150000)
	end
	
	if lv == 100 then
		HelpInfo(role,0,"Поздравляем! Вы достигли 100 уровня, ваш подарок во временном мешке.")
...
------------------------------------------------------
	...
	if ret == LUA_TRUE then
		AddMasterCredit(role, PlayerCredit[lv] * RAID_REP_MENTOR)
		...
		if lv == 41 then
			HelpInfo(role,0,"Поздравляем! Теперь вы квалифицированный наставник! Вы можете обучать собственных учеников. ")
			AddCreditX(role, 100)
			AddMoney(role , 0, 45000)
			GiveItemX( role , 0 , 3345  , 1 , 4 )
			GiveItemX( role , 0 , 3346  , 1 , 4 )
			GiveItemX( role , 0 , 3347  , 1 , 4 )
			GiveItemX( role , 0 , 0855  , 49 , 4 )
			AddMasterCredit(role, 100)
		end
...
-- function MentorsAndDisciples(role)
	...
	-- if CheckTeam == 1 then
		...
		-- LG('MentorsAndDisciples', 'Началась работа функции Ментор и ученик:')
		-- if ChaName == nil or ID == nil then
			-- LG('MentorsAndDisciples', 'Критическая ошибка!!! код: 5')
			-- LG('MentorsAndDisciples', '----------------------------------------------')
			-- return
		-- end
		-- LG('MentorsAndDisciples', 'Имя Ученика: '..ChaName..'(ID = '..ID..')')
		-- local lv_disciples = Lv ( role )
		-- LG('MentorsAndDisciples', 'Ученик совершил подъем уровня до: '..lv_disciples..'.')
		-- local count = 0
		-- local player = {}
		-- player[0] = role
		...
		-- --Сосчитаем количество человек в отряде
		-- for i = 0 , 4 , 1 do
			-- if ValidCha( player[i] ) == 1 then
				...
				-- if star == 1 then
					...
					-- LG('MentorsAndDisciples', 'Лидером отряда является игрок: '..mentor_name..'('..mentor_ID..')')
					-- LG('MentorsAndDisciples', 'Начало проверок ментора и ученика!')
					-- local IsValidTeam = IsValidTeam ( player[i] )
					-- if IsValidTeam == nil then
						-- LG('MentorsAndDisciples', 'Игроки не являются ментором и учеником! Завершение работы функции!')
						-- LG('MentorsAndDisciples', '----------------------------------------------')
						-- return
					-- elseif IsValidTeam == 1 then
						-- LG('MentorsAndDisciples', 'Игроки являются ментором и учеником! Выдача призов ментору!')
						...
						-- end
						-- LG('MentorsAndDisciples', 'Выдача призов заверщена, завершение работы функции!')
						-- LG('MentorsAndDisciples', '----------------------------------------------')
						-- SystemNotice(mentor,'Ваш ученик достиг нового уровня, вы получили награду. Продолжайте в том же духе!')
						-- return
					-- end
				-- end
			-- end
...
function ExAttrCheck(role) 
	...
	local mxhp	= math.floor(Con(role)*3 * Mxhp_con_rad1[job] + Mxhp_con_rad2[job] * math.pow( math.floor(Con(role)*3/20 ),  2) +Lv(role) * Mxhp_lv_rad[job]*RAID_MX_HP + 40)	--Максимальное здоровье
	local mxsp	= math.floor(Sta(role)*3 * Mxsp_sta_rad1[job] + Mxsp_sta_rad2[job] * math.pow( math.floor(Sta(role)*3/20), 2) +Lv(role) * Mxsp_lv_rad[job]*RAID_MX_SP + 5)		--Максимальная мана
	...
	local def = math.floor( Con(role)* 5 * Def_con_rad1[job]	+ Def_con_rad2[job] * math.floor( math.pow( Con(role)*RAID_MX_DEF * 3 /20, 2) ) )								
	local hit = math.floor( Dex(role) * Hit_dex_rad1[job])	+ Lv(role) * 2 + 5		
	local flee = math.floor( Agi(role) * Flee_agi_rad1[job] ) + Lv(role) * 2 + 5			
	local bcolexp = 100
	local mf = 100			--Был ранее = 100 + math.floor( Luk(role)*3 * ----Mf_luk_rad[job] )
	local crt = 15			--Был ранее = 11 + math.floor( Luk(role)*3 * ----Crt_luk_rad[job] )
...
function Ship_ExAttrCheck ( cha_role , ship_role ) 
	...
	if ship_role == nil then 
		LG("Ошибки LUA" , "Ship_ExAttrCheck ship index as null" ) 
		return 0 
	end 
	
	if cha_role == nil then 
		LG ( "Ошибки LUA" , " function Ship_ExAttrCheck :cha_role as null " ) 
		return 
...
function Resume ( role ) 
	...
	if hrec < 0 then 
			...
			LG ( "Ошибки LUA" , "function Resume: character HP recovery rate less than 0" ) 
			return 
	end 
	...
	if role_type == 1 then							
		if hp <= 0 then 
			LG ( "Ошибки LUA" , "function Resume: Character in dead status" ) 
			return 
		end 
		cha_role = GetMainCha ( role ) 
		if sp <= 0 then 
			BickerNotice ( role , "Топливо заканчивается! Судно получает повреждения! Отправляйтесь в ближайший порт! " ) 
			hrec = hrec - 0.020 * mxhp 
			srec = 0 
		end 
		sp = math.max ( 0 , sp - srec + Minus_toplovo ) --Изменил топливо для кораблей
		...
		if ship_lv <= 30 and ship_exp <= 1000  then 
			if a == 1 then 
				...
				SystemNotice (role, ship_expadd.. " Опыта получил корабль ")
