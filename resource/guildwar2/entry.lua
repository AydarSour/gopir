------------------------------------------------------------------------------------------------------------------
--																												--
--					Маленькая священная война entry.lua															--
--					Сервер Mordo Online																			--
--					Версия игры 1.36																			--
...
function after_destroy_entry_guildwar2(entry)
	map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
	Notice("Объявление: Портал в Священную войну для игроков с 45 по 65 уровни закрылся. Удачи!") 
end

function after_player_login_guildwar2(entry, player_name)
	map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
	ChaNotice(player_name, "Объявление: В окрестностях Ледыни ["..posx..","..posy.."] открылся портал в [Священную войну] для игроков с 45 по 65 уровни. Следите за объявлениями. Удачи!") 
end
...
function check_can_enter_guildwar2( role, copy_mgr )
	...
	elseif Lv(role) < 45 or Lv(role) > 64  then
		SystemNotice(role, "Только игроки c 45 по 65 уровни могу войти в [Священную войну]")
	return 0
	end
...
function begin_enter_guildwar2(role, copy_mgr) 
	...
	if GetChaGuildID(Cha) <= 100 and GetChaGuildID(Cha) > 0 then
		if count_haijun2 < 45 then
			...
			SystemNotice(role,"Флотская партия имеет предельное количество бойцов")
		end
	end
	if GetChaGuildID(Cha) > 100 and GetChaGuildID(Cha) <= 200 then
			...
			SystemNotice(role,"Пиратская партия имеет предельное количество бойцов")
		end
	end 
end
