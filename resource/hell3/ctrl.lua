...
function startspecialskill (monsterID)
	...
	local Notice_all = {}
	Notice_all[10] = "Пылай кровь! Мечь фантома!"
	Notice_all[11] = "Гори, Демон огня!"
	Notice_all[12] = "Дай волю своему гневу, Зверь!!!"
	Notice_all[13] = "Разрушай! Душа Тирана!"
	Notice_all[14] = "Воспари в небеса Феникс!!!" 
	Notice_all[15] = "Уничожь их отчаяние!"
	Notice_all[16] = "Проснись раб Дракан"
	Notice_all[17] = "Проснись властитель Ада!!!"
...
function map_copy_run_hell3( map_copy )
	...
	if CheckMonsterDead ( AZRAEL[14] ) == 1 then
		if CRY[14]==0 then
			local Notice_all = "Феникс: Смерть Феникса только начало! Возрожденный Феникс придет и убьет вас всех!"
			MapCopyNotice ( map_copy , Notice_all )
			CRY[14]=1
			BOSSXYSJ[983] = 1
			BOSSSJSJ[983] = 12
	...
	if CRY[10]==1 and CRY[11]==1 and CRY[12]==1 and CRY[13]==1 and CRY[14]==1 and CRY[15]==1 and CRY[16]==1 and CRY[17]==1 then
		if HELLCLOSETIME==300 then
			Notice("Таинственный голос: Я восхищаюсь вашими способностями! Даже Лорды 8 Абаддонов потерпели поражение... У вас есть 3 минуты!")
		...
		for i = 1 , MAXNOTICE , 1 do
			if closetime ==NOTICETIME[i] then
				local Notice_all = "ВНИМАНИЕ, разрушение Абаддонов 10-17 через "..closetime.." секунд! "
				MapCopyNotice ( map_copy ,Notice_all )
			end
		end
