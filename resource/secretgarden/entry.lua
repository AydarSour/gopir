...
function after_create_entry(entry) 
    ...
    Notice("Объявление: Начинается Война гильдий! Обороняющаяся сторона: ["..RedSideName.."]. Атакующая сторона: ["..BlueSideName.."]. Члены обеих гильдий приглашаются на кораблях в море Шайтана ["..posx..","..posy.."], чтобы попасть в [Сад Эдель] для сражения.")
...
function after_player_login_secretgarden(entry, player_name)
    ...
    local BlueSideName = GetGuildName( BlueSide_GuildID ) 
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry)
    ChaNotice(player_name, "Объявление: Начинается Война гильдий! Обороняющаяся сторона: ["..RedSideName.."]. Атакующая сторона: ["..BlueSideName.."]. Члены обеих гильдий приглашаются на кораблях в море Шайтана ["..posx..","..posy.."], чтобы попасть в [Сад Эдель] для сражения.")
end

function check_can_enter_secretgarden( role, copy_mgr )
--проверка на наличие Таинственного замка
	local Num
	Num = CheckBagItem(role,2440)
	if Num >= 1 then
		SystemNotice(role, "У вас в инвентаре Таинственный замок. Невозможно войти в [Сад Эдель] ")	
		return 0
	end
--Таинственный замок
...
function begin_enter_secretgarden(role, copy_mgr) 
	...
	if RedSide_GuildID == GetChaGuildID(Cha) then
		...
		if BlueSide_GuildID == GetChaGuildID(Cha) then
			...
			SystemNotice(role,"Вы вошли в [Сад Эдель]")
