...
function map_copy_run_special_garner2( map_copy ) 
	local HowManyNoDead = GetMapActivePlayer(map_copy)
	Notice("В Серебряном Хаосе все еще есть выжившие: "..HowManyNoDead.." игрок(ов)")
	...
	if Every_5_minute == 4 then -- ёЯј¶±¦Пд
	...
	if Every_5_minute == 6 then -- Т»ЧЄЅЗЙ«
	...
	if Every_5_minute == 7 then -- ¶юЧЄЅЗЙ«
	...
	if Every_5_minute == 8 then -- єЪБъЅЗЙ«
...
--	local time = GetTickCount ( ) 
	...
	if Now_Time == 14 or Now_Time == 20 or Now_Time == 2 then
		return 1
	else
		return 0
	end
...
--	SystemNotice(role,How_Many_Active)
	...
	if How_Many_Active == 1 then
		...
		local message = "Единственный выживший в Серебряном Хаосе ["..cha_name.."] получает ".. Money_Garner2 .." золотых и Эмблему Короля ринга"
	--	SystemNotice(role,How_Many_Active)
...
function kick(role)
	if IsInTeam(role) == 1 then
		...
		SystemNotice(role, "В Серебряном Хаосе нельзя быть в отряде с другими игроками! Вы удалены с карты!")
	end
end