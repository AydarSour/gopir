...
BOSSXYSJ = {}
BOSSXYSJ[979] = 1    -- Барон-фантом
BOSSXYSJ[980] = 12    -- Огненный демон
BOSSXYSJ[981] = 6    -- Злобное отродье
BOSSXYSJ[982] = 4    -- Тайран
BOSSXYSJ[983] = 12    -- Феникс
BOSSXYSJ[984] = 16    -- Отчаяние
BOSSXYSJ[985] = 16    -- Дракан
BOSSXYSJ[986] = 12    -- Тидаль
BOSSXYSJ[987] = 4    -- Хардин
BOSSXYSJ[988] = 4    -- Кара
BOSSXYSJ[789] = 5    -- БД
BOSSXYSJ[807] = 5    --Барба
BOSSXYSJ[805] = 5    --КМД
--BOSSXYSJ[] = 
...
--Авто рейты на выходные
	if AUTO_RAID_MF_START == 1 then
		...
		for i = 1 , diaoliao_count , 1 do 
			if arg[i] >= 100 then 
mf = math.min ( 1, 100 / arg[i] * ( bsmf / 100 ) ) * MF_RAID * MF_RAID_STATE * LVL_DROP * AUTO_RAID_MF * MF_VIP_RAID_NEWBIE * MF_VIP_RAID * RAID_MAP
--perm1 = arg[i]
--perm2 = bsmf / 100
...
function Skill_Hyz_End ( ATKER , DEFER , sklv ) 
	...
	local dmg = ( ( 1 + 0.1 * sklv ) ) * (math.min(3,( math.max ( 1, math.floor( aspd / 85  ) )  ))) * Atk_Dmg ( ATKER , DEFER )
	local map_name_ATKER = GetChaMapName ( ATKER )
	local map_name_DEFER = GetChaMapName ( DEFER )
	...
	local Can_Pk_Garner2 = Is_NormalMonster (DEFER)
		if map_name_ATKER == "garner2" or map_name_DEFER == "garner2" or map_name_ATKER == "07xmas2" or map_name_DEFER == "07xmas2" then
			if Can_Pk_Garner2 == 0 then
			dmg = ( ( 0.7 + 0.1 * sklv ) ) * (math.min(3,( math.max ( 1, math.floor( aspd / 70  ) )  ))) * Atk_Dmg ( ATKER , DEFER ) * 0.90
	--			Notice ( "dmg="..dmg)
			end
		end
	Hp_Endure_Dmg ( DEFER , dmg )  
...
function Skill_Zj_End ( ATKER , DEFER , sklv ) 
	...
	hpdmg = Atk_Raise ( atk_rad , ATKER , DEFER )	* 0.85
	Hp_Endure_Dmg ( DEFER , hpdmg )  
	Check_Ys_Rem ( ATKER ,DEFER )
end 
...
function Skill_Guz_End ( ATKER , DEFER , sklv )
	...
	local statelv = sklv 
	local statetime = 0.5 + math.floor ( sklv * 0.5 ) 
	local map_name_ATKER = GetChaMapName ( ATKER )
	local map_name_DEFER = GetChaMapName ( DEFER )
	local agi_atker = Agi(ATKER)
	local Can_Pk_Garner2 = Is_NormalMonster (DEFER)
	...
	if hp_defer >= 100000 then 
		...
		statetime = math.floor ( statetime+2 / 2 ) + 1 
	end 
	local Check_Heilong = CheckItem_Heilong ( ATKER )
	if Check_Heilong == 1 then
			local Percentage = Percentage_Random ( 0.5 )
...
SystemNotice ( ATKER , "Получена сила набора Черного Дракона. Эффект навыка усилен!")
	...
	if GetChaAIType( DEFER ) >= 21 then
		...
		Hp_Endure_Dmg ( DEFER , hpdmg ) 
		AddState( ATKER , DEFER , STATE_XY, statelv , 3.6 ) 
	else 
		SetSus( DEFER , 0 ) 
	end 
	Check_Ys_Rem ( ATKER ,DEFER )
...
function Skill_Lj_End ( ATKER , DEFER , sklv ) 
	...
	local dmg = math.floor ( 70 + sklv*7 +sta_atk * 5 ) + 3 * Lv
...
function Skill_Xzy_End ( ATKER , DEFER , sklv ) 
	...
	if job == 14 then
		hpdmg = math.floor(hpdmg * 1.4) * 1.1
	else
		hpdmg = hpdmg * 1.1
	end
	Hp_Endure_Dmg ( DEFER , hpdmg ) 
...
function Skill_Xlcz_End ( ATKER , DEFER , sklv ) 
	...
	hpdmg = math.floor (( 10 + sta_atker * 3.6 ) * ( 1 + sklv * 0.2 ) * ( 1 +  lv_dif * 0.02 ))
	local map_name_ATKER = GetChaMapName ( ATKER )
	local map_name_DEFER = GetChaMapName ( DEFER )
	local agi_atker = Agi(ATKER)
	local Can_Pk_Garner2 = Is_NormalMonster (DEFER)
...
function Skill_Sj_End ( ATKER , DEFER , sklv ) 
	...
	if GetChaAIType( DEFER ) >= 21 then    --21ТФЙПОЄBossAIЈ¬Из№ыКЗBossёщѕЭbossІ»Н¬Ј¬Йи¶ЁїЙТФ±»јУЧґМ¬µДґОКэ
...
if rate >= Con(DEFER) then
	local statetime = math.ceil(sklv/10)
	local statelv = sklv
	AddState ( ATKER , DEFER , STATE_XY , statelv , statetime ) 
end
			end
...
function State_KUANGZ_Add ( role , statelv )
	local atksb_dif = 50	* statelv/10
	local def_dif = 30 * statelv/10
	local mnatksb = MnatkSb(role) + atksb_dif   
	local mxatksb = MxatkSb(role ) + atksb_dif 
