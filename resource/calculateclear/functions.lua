	...
	if check == nil then
		...
		local i
			if message == "/n" then
				...
				SystemNotice(role, "Эффект № "..effect)
				PlayEffect (role, 350 ) 

				SystemNotice(role, "Эффект № "..c)
				--
				return 0
			end
...
if map_name_ATKER == "garner" or map_name_DEFER == "garner" or map_name_ATKER == "magicsea" or map_name_DEFER == "magicsea" or map_name_ATKER == "darkblue" or map_name_DEFER == "darkblue" or map_name_ATKER == "binglang2" or map_name_DEFER == "binglang2" or map_name_ATKER == "shalan2" or map_name_DEFER == "shalan2" or map_name_ATKER == "leiting2" or map_name_DEFER == "leiting2" or map_name_ATKER == "eastgoaf" or map_name_DEFER == "eastgoaf" or map_name_ATKER == "jialebi" or map_name_DEFER == "jialebi" or map_name_ATKER == "lonetower" or map_name_DEFER == "lonetower" or map_name_ATKER == "winterland" or map_name_DEFER == "winterland" then
  ...
  local minus = -15 -- Репутация
  
  AddCreditX (ATKER, 20)
  AddCreditX (DEFER, minus)

  SystemNotice ( ATKER ,"Вы победили противника. Получено очков чести: 1. Получено 20 очков репутации ")
  SystemNotice ( DEFER ,"Вы потерпели поражение. Потеряно очков чести: 1. Потеряно 15 очков репутации ")
  RefreshCha (ATKER)
  RefreshCha (DEFER)
  end
	-- Уровень атакующего меньше на 15
  ...
  local minus = -10 -- Репутация
  
  AddCreditX (ATKER, 17)
  AddCreditX (DEFER, minus)

  SystemNotice ( ATKER ,"Вы победили сильного противника. Получено очков чести: 4. Получено 17 очков репутации ")
  SystemNotice ( DEFER ,"Вы потерпели поражение. Потеряно очков чести: 3. Потеряно 10 очков репутации ")
  RefreshCha (ATKER)
  RefreshCha (DEFER)
  end 
	-- Уровень атакующего меньше 16
  ...
  local minus = -25 -- Репутация
  
  AddCreditX (ATKER, 30)
  AddCreditX (DEFER, minus)

  SystemNotice ( ATKER ,"Вы  победили очень сильного противника. Получено очков чести: 6 Получено 30 очков репутации")
  SystemNotice ( DEFER ,"Вы потерпели поражение. Потеряно очков чести: 7 Потеряно 25 очков репутации ")
  RefreshCha (ATKER)
  RefreshCha (DEFER)
  end
	-- Уровень атакующего больше на 15
	...
	if Lv_DEFER==55 or Lv_DEFER==60 then
	  ...
	  local minus = 5 -- Репутация
	  
	  AddCreditX (ATKER, minus)
	  AddCreditX (DEFER, 0)

	  SystemNotice ( ATKER ,"Вы  победили очень слабого противника. Получено очков чести: 1. Потеряно 5 очков репутации ")
	  SystemNotice ( DEFER ,"Вы потерпели поражение. Получено очков чести: 0. Получено 0 очков репутации ")
	  RefreshCha (ATKER)
	  RefreshCha (DEFER)
  ...
  local minus = -3 -- Репутация
  
  AddCreditX (ATKER, 5)
  AddCreditX (DEFER, minus)

  SystemNotice ( ATKER ,"Вы победили слабого противника. Получено очков чести: 1. Получено 5 очков репутации ")
  SystemNotice ( DEFER ,"Вы потерпели поражение. Потеряно очков чести: 1. Потеряно 3 очка репутации ")
  RefreshCha (ATKER)
  RefreshCha (DEFER)
  end
	-- Уровень атакующего больше 20
	if Lv_dif >= 20 then
	--Миники
	if Lv_DEFER==55 or Lv_DEFER==60 then
	  ...
	  SystemNotice ( ATKER ,"Вы  победили очень слабого противника. Потеряно очков чести: 1. Потеряно 5 очков репутации ")
	  SystemNotice ( DEFER ,"Вы потерпели поражение. Получено очков чести: 0. Получено 0 очков репутации ")
	  RefreshCha (ATKER)
	  RefreshCha (DEFER)
  ...
  local minus = -70 -- Репутация
  
  AddCreditX (ATKER, minus)
  AddCreditX (DEFER, 10)

  SystemNotice ( ATKER ,"Вы  победили очень слабого противника. Потеряно очков чести: 8. Потеряно 70 очков репутации ")
  SystemNotice ( DEFER ,"Вы потерпели поражение. Получено очков чести: 2. Получено 10 очков репутации ")
  RefreshCha (ATKER)
  RefreshCha (DEFER)
  end
...
function PK_Dead_Punish_ItemURE ( role )
	...
	if Player == 1 and boat == 0 then
		...
		SystemNotice ( role , "Смерть: потеряно 7% прочности снаряжения" )
	end
end
...
--Захват флага Барбароссы--		
	...
	if posx >= 62 and posx <= 64 and posy >= 47 and posy <= 48 or posx >= 61 and posx <= 62 and posy >= 79 and posy <= 80 or posx >= 43 and posx <= 45 and posy >= 62 and posy <= 63 or posx >= 80 and posx <= 82 and posy >= 62 and posy <= 64 then
		if effect_hp == 0 and effect_hprem == 0 and effect_go == 0 and effect_gorem == 0 and effect_param == 0 and effect_pr == 0 and effect_prrem == 0 and effect_stan == 0 and death == 1 then
			...
			BickerNotice(role,"Вы получили бонусный эффект:" ..BONUS_NAME[rad_bonus]) 
			end
		end
	end
--[[Конец]]--
	...
	if (playerCurrentMap == "garner") or  (playerCurrentMap=="darckblue") or (playerCurrentMap=="magicsea") then
		...
		if(Item_Revive > 0)then -- if has the item
    		if(is_role_living ~= 1)then -- check if player is dead
      		SetRelive(role,role,1,"Хочешь воскреснуть?") -- revive the player
...
--Средний шанс(Равны углы)
	if arg[2] == arg[4] and arg[4] == arg[8] and arg[8] == arg[10] and arg[2] ~= new_id and flag == 1 and flag_myp == 1 then
		SystemNotice ( arg[1] ,"Поздравляем! Вы выиграли 50 000 золотых и комплект снаряжения с Черного рынка и Сундуки Хаоса")
		SynTigerString  ( arg[1] ,"Поздравляем! Вы выиграли 50 000 золотых и комплект снаряжения с Черного рынка и Сундуки Хаоса")
...
--Плохой шанс(Равная диогональ)
	if arg[4] == arg[6] and arg[6] == arg[8] and arg[4] ~= new_id and flag == 1 and flag_myp == 1 then
		...
		if rad_heishi1 == 1 then
			SystemNotice ( arg[1] ,"Поздравляем! Вы выиграли 10 000 золотых и Медь")
			SynTigerString ( arg[1] ,"Поздравляем! Вы выиграли 10 000 золотых и Медь")
			GiveItem ( arg[1] , 0 , 3326 , 1 , 4 )
...
--Плохой шанс(Равная диогональ)
	if arg[2] == arg[6] and arg[6] == arg[10] and arg[2] ~= new_id and flag == 1 and flag_myp == 1 then
		...
		if rad_heishi2 == 1 then
			SystemNotice ( arg[1] ,"Поздравляем! Вы выиграли 5 000 золотых и Сундук Хаоса")
			SynTigerString ( arg[1] ,"Поздравляем! Вы выиграли 5 000 золотых и Сундук Хаоса")
			GiveItem ( arg[1] , 0 , 8535 , 1 , 4 )
			GiveItem ( arg[1] , 0 , 3095 , 1 , 4 )
		else
			SystemNotice ( arg[1] ,"Поздравляем! Вы выиграли 5 000 золотых и Сундук со свитками усиления")
			SynTigerString ( arg[1] ,"Поздравляем! Вы выиграли 5 000 золотых и Сундук со свитками усиления")
