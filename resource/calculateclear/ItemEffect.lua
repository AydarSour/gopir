...
function ItemUse_PKWDYSus ( role , Item )
	...
	local cha_name = GetChaDefaultName ( role ) 
	local message = cha_name.." становится неуязвимым в течение 20 секунд!"
	Notice ( message )
end
...
function Jz_Script_arg(role, Item )
	local i = CheckBagItem(role,8612)
	local k = ChaIsBoat(role)
	local hp = Hp(role)
	local mxhp = Mxhp(role)
	local sp = Sp(role)
	...
	if k == 0 then
		if i > 0 then
			local j = DelBagItem(role,8612,1)
			if j == 1 then
...
function ItemUse_MoreExpGzA( role , Item )
	...
	if DropEXP12[name] == nil then
        DropEXP12[name] = { UsedTime = os.time() }
    end
...
local cooldown = DropEXP12[name].UsedTime - os.time()
	if ChaStateLv == statelv then
		 if cooldown > 0 then
...
local cooldown1 = (cooldown - math.floor(cooldown/60)*60)		 
	...
	DropEXP12[name].UsedTime = os.time()+statetime
...
function ItemUse_MoreExpGzLv3A( role , Item )
	...
	local ChaStateLv = GetChaStateLv ( role , STATE_SBJYGZ )
	local name = GetChaDefaultName(role)
	if DropEXP26[name] == nil then
        DropEXP26[name] = { UsedTime = os.time() }
    end
	if ChaStateLv > statelv then --если уровень этого фрукта меньше уже работающего
		SystemNotice ( role , "Более мощный усилитель опыта еще не окончился " )
		UseItemFailed ( role )
...
local cooldown = DropEXP26[name].UsedTime - os.time()
	if ChaStateLv == statelv then
		 if cooldown > 0 then
local cooldown2 = math.floor(cooldown/60)		 		
local cooldown1 = (cooldown - math.floor(cooldown/60)*60)		 
	...
	DropEXP26[name].UsedTime = os.time()+statetime
	SystemNotice ( role ,"Эффект увеличения опыта активирован " )
...
local cooldown1 = (cooldown - math.floor(cooldown/60)*60)		 
	...
	DropEXP[name].UsedTime = os.time()+statetime
	SystemNotice ( role ,"Эффект увеличения опыта активирован " )end
...
function ItemUse_CJZTQ ( role , Item )
	...
	local ChaStateLv = GetChaStateLv ( role , STATE_SBJYGZ )
	local name = GetChaDefaultName(role)
	if DropEXP2[name] == nil then
        DropEXP2[name] = { UsedTime = os.time() }
    end
	if ChaStateLv > statelv then --если уровень этого фрукта меньше уже работающего
		SystemNotice ( role , "Более мощный усилитель опыта еще не окончился " )
		UseItemFailed ( role )
...
local cooldown = DropEXP2[name].UsedTime - os.time()
	if ChaStateLv == statelv then
		 if cooldown > 0 then
local cooldown2 = math.floor(cooldown/60)		 		
local cooldown1 = (cooldown - math.floor(cooldown/60)*60)		 
	...
	DropEXP2[name].UsedTime = os.time()+statetime
	SystemNotice ( role ,"Эффект увеличения опыта активирован " )
end
...
function ItemUse_MoreExpGzLv3x5A( role , Item )
	...
	local ChaStateLv = GetChaStateLv ( role , STATE_SBJYGZ )
	local name = GetChaDefaultName(role)
	if DropEXP3[name] == nil then
        DropEXP3[name] = { UsedTime = os.time() }
    end
	if ChaStateLv > statelv then --если уровень этого фрукта меньше уже работающего
		SystemNotice ( role , "Более мощный усилитель опыта еще не окончился " )
		UseItemFailed ( role )
...
local cooldown = DropEXP3[name].UsedTime - os.time()
	if ChaStateLv == statelv then
		 if cooldown > 0 then
local cooldown2 = math.floor(cooldown/60)		 		
local cooldown1 = (cooldown - math.floor(cooldown/60)*60)		 
	...
	DropEXP3[name].UsedTime = os.time()+statetime
	SystemNotice ( role ,"Эффект увеличения опыта активирован " )
end
...
function ItemUse_MoreExpGzLv4A( role , Item )
	...
	local ChaStateLv = GetChaStateLv ( role , STATE_SBJYGZ )
	 local name = GetChaDefaultName(role)
	if DropEXP4[name] == nil then
        DropEXP4[name] = { UsedTime = os.time() }
    end
	if ChaStateLv > statelv then --если уровень этого фрукта меньше уже работающего
		SystemNotice ( role , "Более мощный усилитель опыта еще не окончился " )
		UseItemFailed ( role )
...
local cooldown = DropEXP4[name].UsedTime - os.time()
	if ChaStateLv == statelv then
		 if cooldown > 0 then
local cooldown2 = math.floor(cooldown/60)		 		
local cooldown1 = (cooldown - math.floor(cooldown/60)*60)		 
	...
	DropEXP4[name].UsedTime = os.time()+statetime
	SystemNotice ( role ,"Эффект увеличения опыта активирован " )
end
...
local cooldown1 = (cooldown - math.floor(cooldown/60)*60)		 
	...
	DropEXP3[name].UsedTime = os.time()+900
	SystemNotice ( role ,"Эффект увеличения опыта активирован " )
end
...
local cooldown1 = (cooldown - math.floor(cooldown/60)*60)		 
	...
	DropEXP2[name].UsedTime = os.time()+900
	SystemNotice ( role ,"Эффект увеличения опыта активирован " )
end
...
--Фрукт опыта отряда (ID 849)
function ItemUse_ZD_MoreExpGz( role , Item )
 local statelv = 1
 local ChaStateLv = GetChaStateLv ( role , STATE_ZDSBJYGZ )
 local name = GetChaDefaultName(role)
  if EXPparty[name] == nil then
        EXPparty[name] = { UsedTime = os.time() }
    end
local party_num = PartyNum (role)
 if party_num == 1 then
 SystemNotice(role, "Фрукт опыта отряда можно использовать только в отряде!")
  UseItemFailed ( role )
  return
 elseif ChaStateLv > statelv then
  SystemNotice(role, "Фрукт опыта отряда уже используется, дождитесь окончания его эффекта.")
  UseItemFailed ( role )
  return
 end
local rule = IsTeamLeader (role)
if rule < 1 then 
  SystemNotice ( role , "Только глава отряда может использовать фрукт опыта отряда!" )
  UseItemFailed ( role )
  return
end
local cooldown = EXPparty[name].UsedTime - os.time()
 if ChaStateLv == statelv then
   if cooldown > 0 then
local cooldown2 = math.floor(cooldown/60)     
local cooldown1 = (cooldown - math.floor(cooldown/60)*60)   
        SystemNotice(role,"Фрукт опыта отряда уже активен, повторно активировать можно через "..cooldown2.." минут "..cooldown1.." секунд!")
  UseItemFailed ( role )
  return
 end
end
 ...
 EXPparty[name].UsedTime = os.time()+900
 SystemNotice ( role ,"Фрукт опыта отряда активирован " )
end
...
function ItemUse_MonsterSummon ( role , Item , Item_Traget )
 ...
 local map_name = GetChaMapName ( role )
	...
	local now_day=  os.date("%D")		-- Secs Scripts
	 
	if now_hour ~= 21 and now_day ~=6 and now_day~=7 then
		 SystemNotice( role , "Можно использовать только в субботу и воскресенье в 21.00" )
		 UseItemFailed ( role )
		 return
	end
	 ...
	 if map_name =="garner" then
		...
		SystemNotice( role , "Уже призвали Монолит, иди и разруш его!" )
		UseItemFailed ( role )
		return
	 end
	 
	 local MonsterID=1450    
	 local Refresh = 1800
	 local y=280000
	 local x=214900
	 local life = 3600000
	 local new = CreateChaX( MonsterID , x , y , 145 , Refresh, role )
	 SetChaLifeTime( new, life )
	 local message = " Игрок ["..atk_name.."] вызвал Монолит на территории Аскарона по координатам "..x..","..y.."спешите уничтожить его!"  
	 Notice ( message )
		return
		else
	 SystemNotice( role , "Уже призвали Монолит, иди и разруш его!" )
	 ...
	 local life = 3600000
	 local new = CreateChaX( 1450 , 10 , 10 , 145 , Refresh, role )
	 SetChaLifeTime( new, life )
	 local message = " Игрок ["..atk_name.."] вызвал Монолит на территории Магического океана по координатам "..x..","..y.."спешите уничтожить его!"  
	 Notice ( message )
		return
		else
	 SystemNotice( role , "Уже призвали Монолит, иди и разруш его!" )
	 ...
	 local new = CreateChaX( 1450 , 10 , 10 , 145 , Refresh, role )
	 SetChaLifeTime( new, life )
	 local message = " Игрок ["..atk_name.."] вызвал Монолит на территории Великого синего океана по координатам "..x..","..y.."спешите уничтожить его!"  
	 Notice ( message )
		return
		else
	 SystemNotice( role , "Монолит уже призвали на этой территории, иди и разруш его!" )
	 UseItemFailed ( role )
	 return
	 end
...
function ItemUse_FeiFru( role , Item )
	...
	elseif sc <=12400 and sc >6200 then
...
function ItemUse_Buff1( role , Item )
	...
	if Cha_Boat ~= nil then 
		...
		SystemNotice (role, "Вам предоставлены все усиления на 30 минут, наслаждайтесь! ")
	else
...
function ItemUse_Scroll_Str ( role , Item , Item_Traget )
	...
	if ((Item_ID>=8646 and Item_ID<=8705) or (Item_ID==8602) or (Item_ID==8588) or (Item_ID==8581) or (Item_ID==8594) or  (Item_ID>=8563 and Item_ID<=8573)) then
	--Проверка на наличие характеристики
	...
	if rad >= 1 and rad <= 33 then
-----------1 уровень колец ---------
		if Item_ID == 8563 then 
		 GiveItem ( role , 0 , 8646 , 1 , 4 )
		 DelBagItem ( role , 8563 , 1 )
		end
		
		if Item_ID == 8564 then 
		 GiveItem ( role , 0 , 8647 , 1 , 4 )
		 DelBagItem ( role , 8564 , 1 )
		end
		
		if Item_ID == 8565 then 
		 GiveItem ( role , 0 , 8648 , 1 , 4 )
		 DelBagItem ( role , 8565 , 1 )
		end
		
		if Item_ID == 8566 then 
		 GiveItem ( role , 0 , 8649 , 1 , 4 )
		 DelBagItem ( role , 8566 , 1 )
		end
		
		if Item_ID == 8567 then 
		 GiveItem ( role , 0 , 8650 , 1 , 4 )
		 DelBagItem ( role , 8567 , 1 )
		end
		
		if Item_ID == 8568 then 
		 GiveItem ( role , 0 , 8651 , 1 , 4 )
		 DelBagItem ( role , 8568 , 1 )
		end
		
		if Item_ID == 8569 then 
		 GiveItem ( role , 0 , 8652 , 1 , 4 )
		 DelBagItem ( role , 8569 , 1 )
		end
		
		if Item_ID == 8570 then 
		 GiveItem ( role , 0 , 8653 , 1 , 4 )
		 DelBagItem ( role , 8570 , 1 )
		end
		
		if Item_ID == 8571 then 
		 GiveItem ( role , 0 , 8654 , 1 , 4 )
		 DelBagItem ( role , 8571 , 1 )
		end
		
		if Item_ID == 8572 then 
		 GiveItem ( role , 0 , 8655 , 1 , 4 )
		 DelBagItem ( role , 8572 , 1 )
		end
		
		if Item_ID == 8573 then 
		 GiveItem ( role , 0 , 8656 , 1 , 4 )
		 DelBagItem ( role , 8573 , 1 )
		end
		
		if Item_ID == 8588 then 
		 GiveItem ( role , 0 , 8657 , 1 , 4 )
		 DelBagItem ( role , 8588 , 1 )
		end
		
		if Item_ID == 8591 then 
		 GiveItem ( role , 0 , 8659 , 1 , 4 )
		 DelBagItem ( role , 8591 , 1 )
		end
		
		if Item_ID == 8594 then 
		 GiveItem ( role , 0 , 8661 , 1 , 4 )
		 DelBagItem ( role , 8594 , 1 )
		end
		
		if Item_ID == 8602 then 
		 GiveItem ( role , 0 , 8663 , 1 , 4 )
		 DelBagItem ( role , 8602 , 1 )
		end
--------------2 уровень колец -----------

		if Item_ID == 8646 then 
		 GiveItem ( role , 0 , 8665 , 1 , 4 )
		 DelBagItem ( role , 8646 , 1 )
		end
		
		if Item_ID == 8647 then 
		 GiveItem ( role , 0 , 8666 , 1 , 4 )
		 DelBagItem ( role , 8647 , 1 )
		end
		
		if Item_ID == 8648 then 
		 GiveItem ( role , 0 , 8667 , 1 , 4 )
		 DelBagItem ( role , 8648 , 1 )
		end
		
		if Item_ID == 8649 then 
		 GiveItem ( role , 0 , 8668 , 1 , 4 )
		 DelBagItem ( role , 8649 , 1 )
		end
		
		if Item_ID == 8650 then --обязьяна
		 GiveItem ( role , 0 , 8669 , 1 , 4 )
		 DelBagItem ( role , 8650 , 1 )
		end
		
		if Item_ID == 8651 then 
		 GiveItem ( role , 0 , 8670 , 1 , 4 )
		 DelBagItem ( role , 8651 , 1 )
		end
		
		if Item_ID == 8652 then 
		 GiveItem ( role , 0 , 8671 , 1 , 4 )
		 DelBagItem ( role , 8652 , 1 )
		end
		
		if Item_ID == 8653 then 
		 GiveItem ( role , 0 , 8672 , 1 , 4 )
		 DelBagItem ( role , 8653 , 1 )
		end
		
		if Item_ID == 8654 then 
		 GiveItem ( role , 0 , 8673 , 1 , 4 )
		 DelBagItem ( role , 8654 , 1 )
		end
		
		if Item_ID == 8655 then 
		 GiveItem ( role , 0 , 8674 , 1 , 4 )
		 DelBagItem ( role , 8655 , 1 )
		end
		
		if Item_ID == 8656 then 
		 GiveItem ( role , 0 , 8675 , 1 , 4 )
		 DelBagItem ( role , 8656 , 1 )
		end
		
		if Item_ID == 8659 then 
		 GiveItem ( role , 0 , 8660 , 1 , 4 )
		 DelBagItem ( role , 8659 , 1 )
		end
		
		if Item_ID == 8661 then 
		 GiveItem ( role , 0 , 8662 , 1 , 4 )
		 DelBagItem ( role , 8661 , 1 )
		end
		
		if Item_ID == 8663 then 
		 GiveItem ( role , 0 , 8664 , 1 , 4 )
		 DelBagItem ( role , 8663 , 1 )
		end
		
		if Item_ID == 8657 then 
		 GiveItem ( role , 0 , 8658 , 1 , 4 )
		 DelBagItem ( role , 8657, 1 )
		end
--------------3 уровень колец -----------		
		if Item_ID == 8665 then 
		 GiveItem ( role , 0 , 8676 , 1 , 4 )
		 DelBagItem ( role , 8665 , 1 )
		end
		
		if Item_ID == 8666 then 
		 GiveItem ( role , 0 , 8677 , 1 , 4 )
		 DelBagItem ( role , 8666 , 1 )
		end
		
		if Item_ID == 8667 then 
		 GiveItem ( role , 0 , 8678 , 1 , 4 )
		 DelBagItem ( role , 8667 , 1 )
		end
		
		if Item_ID == 8668 then 
		 GiveItem ( role , 0 , 8679 , 1 , 4 )
		 DelBagItem ( role , 8668 , 1 )
		end
		
		if Item_ID == 8669 then --обязьяна
		 GiveItem ( role , 0 , 8680 , 1 , 4 )
		 DelBagItem ( role , 8669 , 1 )
		end
		
		if Item_ID == 8670 then 
		 GiveItem ( role , 0 , 8681 , 1 , 4 )
		 DelBagItem ( role , 8670 , 1 )
		end
		
		if Item_ID == 8671 then 
		 GiveItem ( role , 0 , 8682 , 1 , 4 )
		 DelBagItem ( role , 8671 , 1 )
		end
		
		if Item_ID == 8673 then 
		 GiveItem ( role , 0 , 8684 , 1 , 4 )
		 DelBagItem ( role , 8673 , 1 )
		end
		
		if Item_ID == 8674 then 
		 GiveItem ( role , 0 , 8685 , 1 , 4 )
		 DelBagItem ( role , 8674 , 1 )
		end
		
		if Item_ID == 8675 then 
		 GiveItem ( role , 0 , 8686 , 1 , 4 )
		 DelBagItem ( role , 8675 , 1 )
		end
		
		if Item_ID == 8672 then 
		 GiveItem ( role , 0 , 8683 , 1 , 4 )
		 DelBagItem ( role , 8672 , 1 )
		end
		
		if Item_ID == 8658 then 
		 GiveItem ( role , 0 , 8687 , 1 , 4 )
		 DelBagItem ( role , 8658 , 1 )
		end
		
		if Item_ID == 8660 then 
		 GiveItem ( role , 0 , 8688 , 1 , 4 )
		 DelBagItem ( role , 8660 , 1 )
		end
		
		if Item_ID == 8662 then 
		 GiveItem ( role , 0 , 8689 , 1 , 4 )
		 DelBagItem ( role , 8662 , 1 )
		end
		
		if Item_ID == 8664 then 
		 GiveItem ( role , 0 , 8690 , 1 , 4 )
		 DelBagItem ( role , 8664, 1 )
		end
	...
	--Неудача
	elseif rad >= 34 and rad <= 50 then
		SystemNotice (role , 'Улучшить параметры снаряжения не удалось!' )
...
---------------3 уровень колец-----		
	...
	SystemNotice (role , ' Усиление прошло неудачно, снаряжение потеряно!' )
...
--------------3 уровень колец -----------		
	...
	--Неудача
	elseif rad >= 21 and rad <= 40 then
		SystemNotice (role , 'Улучшить параметры снаряжения не удалось!' )
	
	--Неудача и удаление
	elseif rad >= 41 and rad <= 1000 then
--------------1 уровень колец -----------
		if Item_ID == 8563 then 
		 DelBagItem ( role , 8563 , 1 )
		end
		...
		if i > 0 then
			...
			if j == 1 then
				...
				if map_name_role == "abandonedcity" then 
					...
					if a == 1 then
						GoTo( role,  2816, 3255, "abandonedcity")
						return
					end
				...
				if map_name_role == "abandonedcity2" then 
					...
					if a == 1 then
						GoTo( role,  2816, 3255, "abandonedcity2")
						return
					end
				...
				if map_name_role == "abandonedcity3" then 
					...
					if a == 1 then
						GoTo( role,  2816, 3255, "abandonedcity3")
						return
					end
