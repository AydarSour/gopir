...
TE_GETITEM		= 5			--????
TE_EQUIPITEM	= 6			--????
TE_GOTO_MAP    = 7			--?????	(???????)??1,??ID, ??2,????radom ?,??3,????x,??4, ????y
TE_LEVELUP      	= 8			--??			(???????)??1,??????(????????),??2,?????????,??3,??????
...
function ChekTimeHook(role)
	...
	if antibuy == 0 then
		...
		SystemNotice(role, "Выполнять данное действие можно не чаще чем раз в 5 секунд.")
		return 0
	end
end
...
function AddChaItem2(character,npc,value)
	...
	if cha ==1 then
		SystemNotice( character ,"Вам необходимо сойти на берег, чтобы закончить задание Генезис ")
		return LUA_FALSE 
	end	
	------------???????
	local item1=GetItemP(character,0)
	...
	if itemid1~=2911 then
		SystemNotice( character, "Пожалуйста переместите Письмо с вызовом в первый слот инвентаря." )
		return LUA_FALSE
	end
	----------????
	local now_yes = os.date("%y")			-------------?
	...
	elseif used_time < 2400 and used_time>0 then
		Notice("Наши поздравления Игроку "..cha_name..", он закончил задание Генезис за - "..used_time.." сек!")
	end
	if used_time < JINISI_TIME then
		JINISI_TIME=used_time
		LG( "JiNiSiJiLu_XinXi" , "Игрок "..cha_name.."Broke the best record. Total voyage took"..used_time.." sec(s)!" )
	end
	------------???????
	local r1=0
	local r2=0
...
function LessTime( character, value)
	...
	if HHS_Num ~= 1 then
		SystemNotice( character, "Убедитесь, что в Вашем рюкзаке  есть Письмо молодца ")
		return 0
	end
...
function JINNiuSS(character)
	...
	LG( "JINNiuSS" , "Игрок "..cha_name.."Exchange sailor award" )
	end
...
function JINNiuHD ( character,npc )
	...
	LG( "JINNiuHD" , "Игрок "..cha_name.."Exchange for Pirate prize" )
end
...
function JINNiuCZ ( character,npc )
	...
	LG( "JINNiuCZ" , "Игрок "..cha_name.."Exchange for captain's award" )
end
----LOG---------?????? 
function ShuangZiSS (character,npc)
	...
	LG( "ShuangZiSS" , "Игрок "..cha_name.."Exchange sailor award" )
	end
...
function ShuangZiHD ( character,npc )
	...
	LG( "ShuangZiHD" , "Игрок "..cha_name.."Exchange for Pirate prize" )
end
...
function ShuangZiCZ ( character,npc )
	...
	LG( "ShuangZiCZ" , "Игрок "..cha_name.."Exchange for captain's award" )
end
...
function ZSSTART ( character,npc )
	...
	LG( "ZSSTART" , "Игрок "..cha_name.." The beginning of reincarnation" )
end
...
function ZSSTOP ( character,npc )
	...
	LG( "ZSSTOP" , "Игрок "..cha_name.." The end of reincarnation" )
end

------------------------------------The end of the second reincarnation----dina
function ZSSTOP2 ( character,npc )

	local cha_name = GetChaDefaultName ( character )
	
	LG( "ZSSTOP2" , "Игрок "..cha_name.." закончил Второе перерождение " )
end
...
function CheckBag( character, itemid, value1, value2)-----------value1===??,value2===??
	local cha = ChaIsBoat ( character ) 
	if cha ==1 then
		--SystemNotice( character ,"???????")
		return LUA_TRUE 
...
function CheckBagEmp( character,value)-----------value===??
	if value==nil then
	...
	local cha = ChaIsBoat ( character ) 
	if cha ==1 then
		SystemNotice( character, "\215\242\238\225\251 \225\240\238\241\232\242\252 \226\251\231\238\226 \193\251\242\232\254, \232\227\240\238\234 \236\238\230\229\242 \238\242\239\240\224\226\232\242\252\241\255 \234 \202\238\228\228\232 (2217, 2911). \206\241\226\238\225\238\228\232\242\229 \226 \240\254\234\231\224\234\229 "..item_pos.." \255\247\229\233\234\243. \194\241\229 \232\227\240\238\234\232 \228\238\235\230\237\251 \239\240\232\241\242\224\242\252 \234 \225\229\240\229\227\243")
		return LUA_FALSE 
	end	
	if itemid~=0 then
		SystemNotice( character, "\215\242\238\225\251 \225\240\238\241\232\242\252 \226\251\231\238\226 \193\251\242\232\254, \232\227\240\238\234 \236\238\230\229\242 \238\242\239\240\224\226\232\242\252\241\255 \234 \202\238\228\228\232 (2217, 2911). \206\241\226\238\225\238\228\232\242\229 \226 \240\254\234\231\224\234\229 "..item_pos.." \255\247\229\233\234\243. \194\241\229 \232\227\240\238\234\232 \228\238\235\230\237\251 \239\240\232\241\242\224\242\252 \234 \225\229\240\229\227\243")
...
function SelMissionList( character, npc, selindex, missionlist )
	...
	elseif missionlist[id].tp == RAND_MISSION then
		...
		if ret ~= LUA_TRUE then			
			...
			--else
				--PRINT( "SelMissionList:radom ???????PENDING or DELIVERY??,??????????state = "..state )
				--return SystemNotice( character, "SelMissionList:radom ???????PENDING or DELIVERY??,??????????state = "..state )
			--end
		else
...
function ConditionsTest( character, conditions, param1, param2, npc )
	...
	if conditions[1] == nil then
		...
		for i = 1, num, 1 do --???????????12???
			...
			elseif conditions[i].func == goto_BDL_time then
				PRINT( "ConditionsTest: goto_BDL_time")
				local ret = goto_BDL_time(character ) 
				if ret ~= LUA_TRUE then
					PRINT( "ConditionsTest:goto_BDL_time = false" )
					return LUA_FALSE
				end
			...
			elseif conditions[i].func == ChekTimeHook then
				local ret = ChekTimeHook( character )
				if ret ~= LUA_TRUE then
					return LUA_FALSE
				end
			...
			elseif conditions[i].func == Wedding then
				PRINT( "ConditionsTest:Wedding" )
				local ret = Wedding( character )
				if ret ~= LUA_TRUE then
					PRINT( "ConditionsTest:Wedding = false" )
					return LUA_FALSE
				end
			elseif conditions[i].func == WeddingStop then
				PRINT( "ConditionsTest:WeddingStop" )
				local ret = WeddingStop( character )
				if ret ~= LUA_TRUE then
					PRINT( "ConditionsTest:WeddingStop = false" )
					return LUA_FALSE
				end
			...
			elseif conditions[i].func == GM_TEST then
				PRINT( "ConditionsTest: GM_TEST")
				local ret = GM_TEST(character ) 
				if ret ~= LUA_TRUE then
					PRINT( "ConditionsTest:GM_TEST = false" )
					return LUA_FALSE
				end
			...
			elseif conditions[i].func == CheckRase then
				...
				if ret ~= LUA_TRUE then
					PRINT( "ConditionsTest:CheckRase = false" )
					return LUA_FALSE
				end
	...
	for i = 1, num, 1 do
		...
		elseif actions[i].func == SetTestTime then
			...
			if ret ~= LUA_TRUE then
				...
				SystemNotice( character, "ActionProc: SetTestTime??????failed!" )
				return LUA_FALSE
			end
		elseif actions[i].func == RenewTestTime1 then
			...
			if ret ~= LUA_TRUE then
				...
				SystemNotice( character, "ActionProc: RenewTestTime1??????failed!" )
				return LUA_FALSE
			end
		elseif actions[i].func == RenewTestTime2 then
			...
			if ret ~= LUA_TRUE then
				...
				SystemNotice( character, "ActionProc: RenewTestTime2??????failed!" )
				return LUA_FALSE
			end
		...
		elseif actions[i].func == CreateBoatHook then
			PRINT( "ActionsProc:CreateBoatHook, p1 = , p2 = ", actions[i].p1, actions[i].p2 )
			local ret = CreateBoatHook( character, actions[i].p1, actions[i].p2 )
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:CreateBoatHook failed!" )
				--SystemNotice( character, "ActionsProc:CreateBoatHook failed!" )
				return LUA_FALSE
			end
		...
		elseif actions[i].func == OpenTiger then
			local ret = OpenTiger( character, npc )
			if ret ~= LUA_TRUE then
				return LUA_FALSE
			end
		...
		elseif actions[i].func == GiveNpcMission then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:GiveNpcMission Не удалось!" )
				SystemNotice( character, "ActionProc:GiveNpcMission Не удалось!" )
				return LUA_FALSE
			end
...
function checksjtime( character ,npc )
	...
	elseif time_cjd > 1800 then
		LG("Игрок "..cha_name.."Failed to pass the upgrade exam repeaters! ")
	end
	local del_item =TakeItem( character, 0,3281, 1 )
	if del_item == 0 then
		SystemNotice( character, "\210\229\241\242 \228\238\234\235\224\228\224. \212\238\240\236\224 \232\241\234\235\254\247\224\255\249\224\255 \237\229\241\239\238\241\238\225\237\238\241\242\252 \234 \238\225\237\238\226\235\229\237\232\254")
	end
	return LUA_TRUE
end
...
function XmasNotice ( role, value )				------------------???????
	...
	local message={}
	message[1]="\196\229\228 \204\238\240\238\231: \196\238\225\240\238 \207\238\230\224\235\238\226\224\242\252 \226 \208\238\230\228\229\241\242\226\229\237\241\234\243\254 \196\229\240\229\226\237\254, "..cha_name.." \197\241\235\232 \253\242\238 \239\229\240\226\251\233 \240\224\231, \234\238\227\228\224 \226\251 \239\240\232\229\245\224\235\232 \241\254\228\224, \226\251 \236\238\230\229\242\229 \237\224\233\242\232 \240\255\228\238\236 \241\238 \236\237\238\233 \236\238\229\227\238 \207\238\236\238\249\237\232\234\224 - \238\237 \239\238\236\238\230\229\242 \194\224\236 \239\238\231\237\224\234\238\236\232\242\241\255 \241 \228\229\240\229\226\237\229\233!"
	message[2]="\210\243\231\229\236\246\251: "..cha_name.." \242\224\237\246\243\233 \241 \237\224\236\232, \239\235\255\241\234\232 \226\238\234\240\243\227 \234\238\241\242\240\224 \226\236\229\241\242\229, \237\224\236\237\238\227\238 \226\229\241\229\235\229\229.!"
	message[3]="Cohen said quietly to you: "..cha_name..", Come on, I referred to you to complete the task, so that more of the BOSS Athletic Field comes to Christmas in!"
	message[4]="\204\224\235\229\237\252\234\232\233 \210\238\236: \238\228\237\238\227\238 \226\231\227\235\255\228\224 \237\224 \253\242\238\227\238 \247\229\235\238\226\229\234\224 \236\237\229 \245\226\224\242\232\242, \247\242\238\225\251 \243\231\237\224\242\252 \229\227\238 \232\236\255, \242\229\225\255 \231\238\226\243\242 "..cha_name..",\223 \241\234\238\240\238 \226\251\240\224\241\242\243, \232 \241\242\224\237\243 \237\224\241\242\238\235\252\234\238 \241\232\235\252\237\251\236 \234\224\234 \232 \242\251!"
	SystemNotice( role , message[value] )
	return LUA_TRUE
end

--Автоматическая выдача квестов
function GiveNpcMission( character, id, name, npcmissionid )
	if id == nil or Mission[id] == nil then
		PRINT( "GiveMission: Неверная задача! ID = "..id )
		LG( "mission_error", "GiveMission: Неверная задача! ID = "..id )
		SystemNotice( character, "GiveMission: Неверная задача! ID = "..id )
		return LUA_FALSE
	end
	...
	local npcid = GetCharID( npc )
	if npcid == nil then
		SystemNotice( character, "npcid = 0" )
	end
	PRINT( "GiveMission: npcid, id, mistp", npcid, id, Mission[id].tp )
	--ret = SetMissionTempInfo( character, npcid, id, MIS_ACCEPT, Mission[id].tp )
	ret = SetMissionTempInfo( character, npcid, npcmissionid, MIS_ACCEPT, Mission[id].tp )
	if ret ~= LUA_TRUE then 
		PRINT( "SelMissionList: Временный набор данных не удалось задача!" )
		--SystemNotice( character, "MissionProc: Временный набор данных не удалось задача<<ТЕСТ 2>>!" )
		--SystemNotice( character, "id = "..id )
		--SystemNotice( character, "name = "..name )
		--SystemNotice( character, "npcmissionid = "..npcmissionid )
		return LUA_FALSE
	end
