...
function after_create_entry(entry) 
	...
	Notice("Объявление: В Великом Синем океане ["..posx..","..posy.."] обнаружен портал, ведущий в [Мираж Ледыни].")
end

function after_destroy_entry_binglang2(entry)
	map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
	Notice("Объявление: Портал в [Мираж Ледыни] закрылся. Удачи!") 
end

function after_player_login_binglang2(entry, player_name)
	map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry)
	ChaNotice(player_name, "Объявление: В Великом Синем океане ["..posx..","..posy.."] обнаружен портал, ведущий в [Мираж Ледыни].")
end
...
function check_can_enter_binglang2( role, copy_mgr )
	...
	if Lv(role) > 100 then
		SystemNotice(role, "Вы должны быть не выше 100 уровня, чтобы войти в [Мираж Ледыни]")
		return 0    
	end
